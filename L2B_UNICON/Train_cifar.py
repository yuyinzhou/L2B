...
def l2b(net, optimizer, image, labels, metaloader):
    ...
    with higher.innerloop_ctx(net, optimizer) as (meta_net, meta_optimizer):
        ...
        if args.need_clean:
            ...
            val_data, inputs_x2, inputs_x3, inputs_x4, val_labels, w_x = next(iter(metaloader))

        # val_data, val_labels = next(iter(metaloader))
        val_data = val_data.cuda()
        val_labels = val_labels.cuda()

        _, y_g_hat = meta_net(val_data)
    ...
    if args.dataset =='cifar10':
        ...
        norm = 0.80
    nn.utils.clip_grad_norm_(net.parameters(), norm, norm_type=2)
    optimizer.step()
    return  net
...
meta_loader= dataloader.cifar_dataset(args.dataset,r=args.r,root_dir=model_save_loc,noise_mode=args.noise_mode, sample_ratio=1.0,
                                      transform=transform_train, mode='meta', noise_file='')
