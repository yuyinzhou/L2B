    ...
    def run(self, sample_ratio, mode, pred=[], prob=[]):
        if mode=='warmup':
            ...
            unlabeled_trainloader = DataLoader(
                ...
                batch_size= int(self.batch_size/(2*sample_ratio)) if self.r<=0.5 else int(self.batch_size / (sample_ratio)),
                #batch_size=int(self.batch_size / (sample_ratio)), # for high noise ratio
                shuffle=True,
                num_workers=self.num_workers, drop_last =True)    
