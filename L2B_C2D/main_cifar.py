...
def parse_args():
    ...
    parser.add_argument('--use_l2b', dest='use_l2b', action='store_true', help='use l2b for training')
    parser.add_argument('--need_clean', dest='need_clean', action='store_true', help='need clean')
    parser.add_argument('--num_meta', default=500, type=int)
...
def create_model_bit(net='resnet18', dataset='cifar100', num_classes=100, device='cuda:0', drop=0):
    if net == 'resnet50':
        ...
        model.load_from(np.load("/home/ec2-user/disk/pretrained/pretrained/BiT-S-R50x1.npz"))
        model = model.to(device)
    elif net == 'resnet18':
...
def main():
    args = parse_args()
    os.makedirs('/home/ec2-user/disk/checkpoint', exist_ok=True)
    log_name = '/home/ec2-user/disk/checkpoint/%s_%s_%.2f_%.1f_%s' % (
    ...
    if args.use_l2b:
        print('using metadata for bootstrapping')
        ...
        meta_data= dataloader.cifar_dataset(args.dataset,
                                               r=args.r, root_dir=args.data_path, noise_mode=args.noise_mode,
    ...
    sched1 = torch.optim.lr_scheduler.StepLR(optimizer1, args.step, gamma=0.1)
    sched2 = torch.optim.lr_scheduler.StepLR(optimizer2, args.step, gamma=0.1)
    ...
    run_train_loop(net1, optimizer1, sched1, net2, optimizer2, sched2, criterion, CEloss, CE, loader, args.p_threshold,
                   ...
                   args.alpha, args.noise_mode, args.dataset, args.r, conf_penalty, stats_log, loss_log, test_log, args, metadata=meta_loader)
