...
class cifar_dataset(Dataset):
    def __init__(self, dataset, r, noise_mode, root_dir, transform, mode, noise_file='', pred=[], probability=[],
                 log='', oracle='none', mix_labelled=True, num_meta=500):
        ...
        if self.mode == 'test':
            ...
            if self.mode == 'all':
                ...
                idx = list(range(50000))
                random.shuffle(idx)
                meta_id = idx[0:num_meta]
                print(f'using {num_meta} data as metaset')
...
class cifar_dataloader():
    def __init__(self, dataset,
        ...
        self.clean = cifar_dataset(dataset=self.dataset, noise_mode=self.noise_mode, r=self.r,
                                   root_dir=self.root_dir, transform=self.transform_warmup,
                                   mode="all",
                                   num_meta=num_meta,
    ...
    def run(self, mode, pred=[], prob=[]):
        if mode == 'warmup':
            all_dataset = cifar_dataset(dataset=self.dataset, noise_mode=self.noise_mode, r=self.r, num_meta=self.num_meta,
